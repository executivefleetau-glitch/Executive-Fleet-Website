 ...
 */
export function driverEmailTemplate(booking, adminMessage) {
  // Format the pickup date
  let formattedDate = 'N/A';
  try {
    const d = new Date(booking.pickupDate);
    if (!isNaN(d.getTime())) {
      formattedDate = d.toLocaleDateString('en-AU', {
        weekday: 'long',
        day: 'numeric',
        month: 'long',
        year: 'numeric',
  ...
  try {
    const t = new Date(booking.pickupTime);
    if (!isNaN(t.getTime())) {
      formattedTime = t.toLocaleTimeString('en-AU', {
        hour: 'numeric',
        minute: '2-digit',
        hour12: true,
        timeZone: 'Australia/Melbourne',
  ...
  // Format return date/time if applicable
  ...
  if (booking.isReturnTrip) {
    try {
      ...
      if (!isNaN(rd.getTime())) {
        formattedReturnDate = rd.toLocaleDateString('en-AU', {
          weekday: 'long',
          day: 'numeric',
          month: 'long',
          year: 'numeric',
    ...
    try {
      const rt = new Date(booking.returnTime);
      if (!isNaN(rt.getTime())) {
        formattedReturnTime = rt.toLocaleTimeString('en-AU', {
          hour: 'numeric',
          minute: '2-digit',
          hour12: true,
          timeZone: 'Australia/Melbourne',
